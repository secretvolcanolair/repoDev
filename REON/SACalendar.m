...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row < firstDay || indexPath.row >= firstDay + daysInMonth) {
        ...
        //--- If this date is in the dateArray, then display gray circle
        else if([self ifEventPresentInMonth:month day:(indexPath.row - firstDay + 1) year:current_year]){
...
-(BOOL) ifEventPresentInMonth: (NSInteger)mnth day:(NSInteger)dy year:(NSInteger)yr {
    
    for(NSMutableDictionary *dateObject in _eventArray){
        
        NSNumber *monthNumber = (NSNumber *)[dateObject valueForKey:@"month"];
        NSNumber *dayNumber = (NSNumber *)[dateObject valueForKey:@"day"];
        NSNumber *yearNumber = (NSNumber *)[dateObject valueForKey:@"year"];
        
        NSInteger monthInteger = (NSInteger)[monthNumber integerValue];
        NSInteger dayInteger = (NSInteger)[dayNumber integerValue];
        NSInteger yearInteger = (NSInteger)[yearNumber integerValue];
        
        if(monthInteger == mnth && dayInteger == dy && yearInteger == yr){
            return YES;
        }
