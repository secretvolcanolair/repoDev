...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.row >= totalBlankDays){
        ...
        BOOL sunCheck = (((int)indexPath.row + 7)%7)==0 ? YES : NO;
        BOOL satCheck = (((int)indexPath.row + 1)%7)==0 ? YES : NO;
        
        if(sunCheck||satCheck){
            cell.dayLabel.textColor = [UIColor lightGrayColor];
        }else{
            cell.dayLabel.textColor = [UIColor blackColor];
        }
        
        //--- Check if cell is today's day
        if(_month == _currentMonth && _year == _currentYear && _currentDay == (int)(indexPath.row+1)-totalBlankDays){
            [cell setAsTodaysDay];
        }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(sunCheck){
        return CGSizeMake(50, 66.0);
    }else{
        return CGSizeMake(45.0, 66.0);
    }
